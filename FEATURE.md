...
**Goal:** Score every file _not_ in the Seed Basket to determine its relevance, then greedily select the best ones based on a token budget.
...
To manage the complexity of the **Dynamic & Intelligent Context Builder**, the feature will be developed in seven distinct, incremental stages. Each stage results in a shippable application, progressively adding layers of intelligence and functionality. This approach ensures continuous integration and allows for testing and refinement throughout the development cycle.

#### **Stage 1: Foundational Backend Services**

- **Goal:** Establish the core, non-UI backend services required for context analysis.
- **Implementation:**
  - Create a `GitService.ts` to interface with the local Git repository for commit history analysis.
  - Create a stateless `DependencyScanner.ts` for fast, regex-based dependency scanning across multiple languages.
- **Outcome:** Backend infrastructure is in place and unit-testable. The application remains fully functional with no visible changes.

#### **Stage 2: Three-Tier UI Driven by Manual File Selection**

- **Goal:** Implement the user-facing three-tier context model (`Selected`, `Neighboring`, `Other`) driven solely by the user's manual file selections.
- **Implementation:**
  - Create a basic `RelevanceEngineService.ts` that identifies direct dependencies of selected files as the "neighboring" context.
  - Update the `FileExplorerItem.tsx` component to visually render the three tiers.
  - Update `buildPrompt.ts` to consume the new context object (full content for selected files, smart previews for neighboring files).
- **Outcome:** The feature becomes visible and interactive. The File Explorer now provides feedback based on file selection, and the prompt builder is more intelligent, but the context only updates on manual user interaction.

#### **Stage 3: Live Task Description Analysis**

- **Goal:** Make the context builder dynamic by making it react to the user's written intent in real-time.
- **Implementation:**
  - Enhance `RelevanceEngineService.ts` to process the `taskDescription` text and implement the "Task Keywords" heuristic.
  - Wire up the `ActionPanel.tsx` to trigger a debounced context recalculation as the user types in the task description area.
- **Outcome:** The UI now feels "live." The File Explorer updates automatically as the user types, providing immediate feedback on how their task description is influencing the context.

#### **Stage 4: Comprehensive Scoring Engine**

- **Goal:** Massively upgrade the context builder's intelligence by implementing the full, multi-faceted scoring algorithm.
- **Implementation:**
  - Flesh out `RelevanceEngineService.ts` with the complete two-phase scoring logic defined in the specification.
  - Integrate heuristics that rely on the foundational services, such as `Shared Commits`, `Sibling File`, and `Folder Co-location`.
- **Outcome:** The quality of the "neighboring" context suggestions improves dramatically. The system now uses a sophisticated, weighted model to determine relevance, transparently enhancing the user experience.

#### **Stage 5: Advanced Graph-Based Analysis**
...
- **Goal:** Ensure the advanced analysis from Stage 5 is performant and does not slow down application startup or use.
- **Implementation:**
...
#### **Stage 7: Live Activity & User Configuration**
